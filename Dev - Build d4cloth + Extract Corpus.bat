...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "cmake -S tools/d4cloth -B tools/d4cloth/build -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE='%VCPKG_ROOT%/scripts/buildsystems/vcpkg.cmake' -DVCPKG_TARGET_TRIPLET=x64-windows '-DVCPKG_INSTALL_OPTIONS=--x-buildtrees-root=%USERPROFILE%/vbt' 2>&1 | Tee-Object -FilePath '%~dp0d4cloth_build_log.txt'; exit $LASTEXITCODE"
if not "%errorlevel%"=="0" (
